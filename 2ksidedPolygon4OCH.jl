...
@inline function alg_2ksidePolygon4OCH(X,k)
...
@inline function alg_4ksidePolygon4OCH_parallel_ex(X,k, exportFile)
    ohull = alg_2ksidePolygon4OCH(X,k)
    exportResult(ohull, exportFile)
end

X = create_discs(1000)
ohull = @time alg_2ksidePolygon4OCH(X,8)
