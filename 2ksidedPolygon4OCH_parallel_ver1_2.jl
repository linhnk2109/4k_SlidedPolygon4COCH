...
using Base.Threads
include("utils.jl")
@inline function starting_vertices_parallel_ver1_2(X,k)
    Is = [[] for _ in 1:(4k+1)]
    len = size(X,1)
    ...
    @threads for j in 1:4k
        ...
        elseif (j>1 && j <=k) || (j>k+1 && j<=2k)
            ...
            Is[j] = [p]
        elseif (j>2k+1 && j <=3k) || (j>3k+1 && j<=4k)
            d = pi*(j-2k-1)/2k
            e_x = cos(d)
            e_y = sin(d)
...
@inline function find_P_parallel_ver1_2(X, Is,k)
    Xx = X[:,1]
    Xy = X[:,2]
...
@inline function o_1_parallel_ver1_2(P)
...
@inline function o_2_parallel_ver1_2(P)
    P = sortslices(P, dims = 1, by = x -> x[1])
    ohull = [P[1,:]]
    x_max = P[1,1]
    y_max = P[1,2]
...
@inline function o_3_parallel_ver1_2(P)
    P = sortslices(P, dims = 1, by = x -> x[1])
    ohull = [P[1,:]]
    x_max = P[1,1]
    y_min = P[1,2]
...
@inline function o_4_parallel_ver1_2(P)
    P = sortslices(P, dims = 1, by = x -> -x[1])
    ohull = [P[1,:]]
    x_min = P[1,1]
    y_min = P[1,2]
...
@inline function alg_2ksidePolygon4OCH_parallel_ver1_2(X,k)
    Is = starting_vertices_parallel_ver1_2(X,k)
    P = find_P_parallel_ver1_2(X,Is,k)
    V = [[] for _ in 1:4k]
    @threads for i in 1:4k
        if i > 0 && i <= k
            V[i] = o_1_parallel_ver1_2(P[i])
        elseif i > k && i <= 2k
            V[i] = o_2_parallel_ver1_2(P[i])
        elseif i > 2k && i <= 3k
            V[i] = o_3_parallel_ver1_2(P[i])
        else
            V[i] = o_4_parallel_ver1_2(P[i])
        end
...
@inline function alg_2ksidePolygon4OCH_parallel_ex_ver1_2(X,k, exportFile)
    ohull = alg_2ksidePolygon4OCH_parallel_ver1_2(X,k)
    exportResult(ohull, exportFile)
end

X = create_discs(1000)
ohull = @time alg_2ksidePolygon4OCH_parallel_ver1_2(X,8)
