...
@inline function starting_vertices_parallel_ver2_2(X,k, startIndex, endIndex)
    Is = [[] for _ in 1:4k]
    for j in 1:2k
        if j ==1 || j == k+1 
            Is[j] = [[X[startIndex,1],X[startIndex,2]],[X[startIndex,1],X[startIndex,2]]]
            Is[j+2k] = [[X[startIndex,1],X[startIndex,2]],[X[startIndex,1],X[startIndex,2]]]
        else
            Is[j] = [[X[startIndex,1],X[startIndex,2]]]
            Is[j+2k] = [[X[startIndex,1],X[startIndex,2]]]
        end        
    end
    ...
    for i in startIndex+1:endIndex
        ...
        for j in 1:2k 
            ...
            e_x = cos(d)
            e_y = sin(d)
            max_j = e_x*Is[j][1][1]+e_y*Is[j][1][2]
            min_j = e_x*Is[j+2k][1][1]+e_y*Is[j+2k][1][2]
            t = e_x*x+ e_y*y
            if j == 1
                p_max = Is[j][2]
                p_min = Is[j][1]
...
@inline function starting_vertices_parallel_ver2_2(X,k)
    len = size(X,1)
    grid = mySplit(1,len, nthreads())
    result = [[] for _ in 1:(length(grid)-1)]
    @threads for i in 1:(length(grid)-1)
        result[i] = starting_vertices_parallel_ver2_2(X,k,grid[i],grid[i+1]-1)
    end
    Is = result[1]
    for i in 2:(length(grid)-1)
        for j in 1:2k 
            ...
            elseif j == k+1
                ...
                p_min[1] = max_j_lt_t ? x_min : p_min[1]
                p_max[2] = max_j_lt_t ? y_max : p_max[2]
                p_min[2] = max_j_lt_t ? y_min : p_max[2]
                ...
                #I^(j+2k)
                p2 = Is[j+2k][1]
                x2 = result[i][j+2k][1][1]
                y2 = result[i][j+2k][1][2]
                t2 = e_x*x2+ e_y*y2
                min_j_gt_t =  min_j > t2
    
                p2[1] = min_j_gt_t ? x2 : p2[1]
                p2[2] = min_j_gt_t ? y2 : p2[2]
...
@inline function find_P_parallel_ver2_2(X, Is,k)
    Xx = X[:,1]
    Xy = X[:,2]
...
@inline function o_1_parallel_ver2_2(P)
...
@inline function o_2_parallel_ver2_2(P)
    P = sortslices(P, dims = 1, by = x -> x[1])
    ohull = [P[1,:]]
    x_max = P[1,1]
    y_max = P[1,2]
...
@inline function o_3_parallel_ver2_2(P)
    P = sortslices(P, dims = 1, by = x -> x[1])
    ohull = [P[1,:]]
    x_max = P[1,1]
    y_min = P[1,2]
...
@inline function o_4_parallel_ver2_2(P)
    P = sortslices(P, dims = 1, by = x -> -x[1])
    ohull = [P[1,:]]
    x_min = P[1,1]
    y_min = P[1,2]
...
@inline function alg_2ksidePolygon4OCH_parallel_ver2_2(X,k)
    Is = starting_vertices_parallel_ver2_2(X,k)
    P = find_P_parallel_ver2_2(X,Is,k)
    V = [[] for _ in 1:4k]
    @threads for i in 1:4k
        if i > 0 && i <= k
            V[i] = o_1_parallel_ver2_2(P[i])
        elseif i > k && i <= 2k
            V[i] = o_2_parallel_ver2_2(P[i])
        elseif i > 2k && i <= 3k
            V[i] = o_3_parallel_ver2_2(P[i])
        else
            V[i] = o_4_parallel_ver2_2(P[i])
        end
...
@inline function alg_2ksidePolygon4OCH_parallel_ex_ver2_2(X,k, exportFile)
    ohull = alg_2ksidePolygon4OCH_parallel_ver2_2(X,k)
    exportResult(ohull, exportFile)
end

X = create_discs(1000)
ohull = @time alg_2ksidePolygon4OCH_parallel_ver2_2(X,8)
