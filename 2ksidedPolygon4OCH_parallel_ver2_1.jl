...
@inline function starting_vertices_parallel_ver2_1(X,k, startIndex, endIndex)
    Is = [[] for _ in 1:4k]
    for j in 1:2k
        d = pi*(j-1)/2k
        e_x = cos(d)
        ...
        elseif j == k+1
            ...
            p = [X[startIndex,1],X[startIndex,2]]

            p2 = [X[startIndex,1],X[startIndex,2]]
...
@inline function starting_vertices_parallel_ver2_1(X,k)
    len = size(X,1)
    grid = mySplit(1,len, 64*nthreads())
    result = [[] for _ in 1:(length(grid)-1)]
    @threads for i in 1:(length(grid)-1)
        result[i] = starting_vertices_parallel_ver2_1(X,k,grid[i],grid[i+1]-1)
    end
    Is = result[1]
...
@inline function find_P_parallel_ver2_1(X, Is,k)
    Xx = X[:,1]
    Xy = X[:,2]
...
@inline function o_1_parallel_ver2_1(P)
...
@inline function o_2_parallel_ver2_1(P)
    P = sortslices(P, dims = 1, by = x -> x[1])
    ohull = [P[1,:]]
    x_max = P[1,1]
    y_max = P[1,2]
...
@inline function o_3_parallel_ver2_1(P)
    P = sortslices(P, dims = 1, by = x -> x[1])
    ohull = [P[1,:]]
    x_max = P[1,1]
    y_min = P[1,2]
...
@inline function o_4_parallel_ver2_1(P)
    P = sortslices(P, dims = 1, by = x -> -x[1])
    ohull = [P[1,:]]
    x_min = P[1,1]
    y_min = P[1,2]
...
@inline function alg_2ksidePolygon4OCH_parallel_ver2_1(X,k)
    Is = starting_vertices_parallel_ver2_1(X,k)
    P = find_P_parallel_ver2_1(X,Is,k)
    V = [[] for _ in 1:4k]
    @threads for i in 1:4k
        if i > 0 && i <= k
            V[i] = o_1_parallel_ver2_1(P[i])
        elseif i > k && i <= 2k
            V[i] = o_2_parallel_ver2_1(P[i])
        elseif i > 2k && i <= 3k
            V[i] = o_3_parallel_ver2_1(P[i])
        else
            V[i] = o_4_parallel_ver2_1(P[i])
        end
...
@inline function alg_2ksidePolygon4OCH_parallel_ex_ver2_1(X,k, exportFile)
    ohull = alg_2ksidePolygon4OCH_parallel_ver2_1(X,k)
    exportResult(ohull, exportFile)
end

X = create_discs(1000)
ohull = @time alg_2ksidePolygon4OCH_parallel_ver2_1(X,8)
